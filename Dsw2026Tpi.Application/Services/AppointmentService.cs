...
public class AppointmentService : IAppointmentService
{
    ...
    public async Task<Pagination<AppointmentSearchResponse>> Search(Guid? specialtyId, Guid? doctorId, long? dni, DateTime? date, int pageSize, int pageIndex)
    {
        ...
        if (specialtyId.HasValue)
            query = query.Where(a => a.AvailabilitySlot != null && a.AvailabilitySlot.Doctor != null && a.AvailabilitySlot.Doctor.SpecialityId == specialtyId.Value);

        if (doctorId.HasValue)
            query = query.Where(a => a.AvailabilitySlot != null && a.AvailabilitySlot.DoctorId == doctorId.Value);

        if (dni.HasValue)
            query = query.Where(a => a.Patient != null && a.Patient.Dni == dni.Value);

        if (date.HasValue)
        {
            var start = date.Value.Date;
            var end = date.Value.Date.AddDays(1);
            query = query.Where(a => a.AvailabilitySlot != null && a.AvailabilitySlot.StartTime >= start && a.AvailabilitySlot.StartTime < end);
        }

        var ordered = query.OrderBy(a => a.AvailabilitySlot != null ? a.AvailabilitySlot.StartTime : DateTime.MaxValue);
        ...
        {
            Specialty = a.AvailabilitySlot != null && a.AvailabilitySlot.Doctor != null && a.AvailabilitySlot.Doctor.Speciality != null ? a.AvailabilitySlot.Doctor.Speciality.Name : string.Empty,
            Doctor = a.AvailabilitySlot != null && a.AvailabilitySlot.Doctor != null ? a.AvailabilitySlot.Doctor.Name : string.Empty,
            AvailableTime = a.AvailabilitySlot != null ? a.AvailabilitySlot.StartTime : DateTime.MinValue
        }).ToList();
